...
rDownloadURL = {"main": "https://www.dropbox.com/s/wxxfifeun3899jl/main_xtreamcodes_reborn.tar.gz?dl=0", "sub": "https://www.dropbox.com/s/7zrly39tlco12bv/sub_xtreamcodes_reborn.tar.gz?dl=0"}
...
def prepare(rType="MAIN"):
    ...
    os.system("wget -q -O /tmp/libpng12.deb https://www.dropbox.com/s/qtmi7ficdchso6r/libpng12-0_1.2.54-1ubuntu1_amd64.deb?dl=0")
...
def mysql(rUsername, rPassword):
    ...
    printc("Enter MySQL Root Password: type anything here", col.WARNING)
    for i in range(5):
        rMySQLRoot = raw_input("  ")
        print " "
        if len(rMySQLRoot) > 0: rExtra = " -p%s" % rMySQLRoot
        else: rExtra = ""
        printc("Drop existing & create database? To clean installation always type Y - Y/N", col.WARNING)
...
def configure():
    ...
    os.system('apt-get install unzip e2fsprogs python-paramiko -y && chattr -i /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb && rm -rf /home/xtreamcodes/iptv_xtream_codes/admin 2>/dev/null && rm -rf /home/xtreamcodes/iptv_xtream_codes/pytools 2>/dev/null && wget -q "https://www.dropbox.com/s/ldf5qecj63809g7/update.zip?dl=0" -O /tmp/update.zip -o /dev/null && unzip /tmp/update.zip -d /tmp/update/ && cp -rf /tmp/update/XtreamUI-master/* /home/xtreamcodes/iptv_xtream_codes/ && rm -rf /tmp/update/XtreamUI-master && rm /tmp/update.zip && rm -rf /tmp/update  && chown -R xtreamcodes:xtreamcodes /home/xtreamcodes/ && chmod +x /home/xtreamcodes/iptv_xtream_codes/permissions.sh && /home/xtreamcodes/iptv_xtream_codes/permissions.sh && find /home/xtreamcodes/ -type d -not \( -name .update -prune \) -exec chmod -R 777 {} +')
    os.system("sed -i 's|echo \"MODS NG ODIN https://discord.gg/mH6D7VWXmt \";|header(\"Location: https://www.google.com/\");|g' /home/xtreamcodes/iptv_xtream_codes/wwwdir/index.php")
    os.system("sudo wget -q https://www.dropbox.com/s/etdx2f5emxsl630/youtube-dl?dl=0 -O /usr/local/bin/youtube-dl")
    os.system("sudo chmod a+rx /usr/local/bin/youtube-dl")

def start(first=True):
    if first: printc("Starting CK Mods NG Odin")
    else: printc("Restarting CK Mods NG Odin")
...
if __name__ == "__main__":
    printc("CK MODS NG Odin - CLEAN Ubuntu 18.04 Installer", col.OKGREEN, 2)
    ...
    if rType.upper() in ["MAIN", "LB"]:
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            ...
            if raw_input("  ").upper() == "Y":
                ...
                start()
                printc("CK MODS NG Odin completed!", col.OKGREEN, 2)
                printc("Your admin UI Access: http://%s:25500" % getIP())
                if rType.upper() == "MAIN":
